...
static char g_61bit[28+1]="123456";//原始交易信息
static char g_35bit[37+1];
...
int main(int argc, char *argv[])
{
    ...
    cmd_init("moniSalecmd>>",TransRcvSvrExit);
  	cmd_add("moniChexiao",     moniChexiao,   " moniChexiao");
  	cmd_add("SetDatePara",     SetDatePara,   " SetDatePara");
  	cmd_add("SetTracePara",     SetTracePara,   " SetTracePara");
  	cmd_add("Set61bit",     Set61bit,   " Set61bit");
  	cmd_add("Set35bit",     Set35bit,   " Set35bit");
 	cmd_shell(argc,argv);
...
int GetTrace(char *trace, char *date, PEP_JNLS *pep_jnls)
{
    ...
    dcs_log(0, 0, "原交易查询");
  	...
  	sprintf(sql, "SELECT trace, outcdno, tranamt, poitcde, poscode, termcde, mercode, sendcde,trnsndp,translaunchway,samid, \
  			postermcde,posmercode,termtrc,batchno,process,trancde,card_sn FROM pep_jnls WHERE trace ='%s' and pepdate= '%s' and trancde not like 'C%';", trace, date);
    ...
    if(num_rows == 1) {
		...
		memcpy(pep_jnls->card_sn, row[17] ? row[17] : "\0", (int)sizeof(pep_jnls->card_sn));
	}
...
int moniChexiao()
{
	...
	dcs_log(0,0," 模拟POS消费撤销交易发送给核心 start");//0,1,3,4,11,22,25,26,35,36,41,42,49,52,53,60,64
	time(&t);
	tm = localtime(&t);
	...
	pub_get_trace_from_seq(trace);
	setbit(&siso,11,(unsigned char *)trace, 6);//流水
	...
	if(memcmp(pep_jnls.trnsndp, "TPOS", 4) ==0 )//传统POS
	{
			...
			if(memcmp(T0_flag, "0", 1) ==0 )
			{
				memset(angentcode, 0, sizeof(angentcode));
				memcpy(angentcode, "000000", 6);
			}
	...
	setbit(&siso,35,(unsigned char *)g_35bit, strlen(g_35bit));
	setbit(&siso,37,(unsigned char *)g_61bit+6, 12);

	if(memcmp(pep_jnls.authcode, "000000", 6) != 0)
		setbit(&siso,38,(unsigned char *)g_61bit, 6);
	...
	//组61域
//	memset(buffer, 0, sizeof(buffer));
//	sprintf(buffer, "%s%s", g_61bit,g_trace);
	setbit(&siso,61,(unsigned char *)g_61bit , strlen(g_61bit));
