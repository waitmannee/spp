...
static struct http_Ast *s_http_Ctrl = NULL;
static void Syn_Http_Cmd_Exit(int signo);
static int Add_Syn_Http_Link();
static void List_Syn_Http_Info(int arg_c, char *arg_v[]);
static int Reload_Syn_Http_Cfg(struct http_Ast *pLink,char *pname,int mode);
static int Stop_Syn_Http_Link();
static int Get_Syn_Http_Link(char *pname,int *pFlag);
static int Open_Syn_Http_Cmd_Log();
static int  Start_Syn_Http_Link();
static int Update_Syn_Http_Link();
//==========================================

int main(int argc, char *argv[])
{
	catch_all_signals(Syn_Http_Cmd_Exit);

	if(Open_Syn_Http_Cmd_Log()< 0)
		return -1;
	
	if(s_http_Ctrl == NULL)
	{ 
		s_http_Ctrl = (struct http_Ast*)shm_connect(SYN_HTTP_SHM_NAME, NULL);
		if(s_http_Ctrl == NULL)
		{
	...
	cmd_init("synHttpCmd>>",Syn_Http_Cmd_Exit);
	cmd_add("list", (cmdfunc_t *)List_Syn_Http_Info, "list link info");
	cmd_add("start", (cmdfunc_t *)Start_Syn_Http_Link, "start a link");
	cmd_add("add", (cmdfunc_t *)Add_Syn_Http_Link, "Add a link ");
	cmd_add("stop", (cmdfunc_t *)Stop_Syn_Http_Link, "stop a link");
	cmd_add("update", (cmdfunc_t *)Update_Syn_Http_Link, "update  a link");
...
static void List_Syn_Http_Info(int arg_c, char *arg_v[])
{
	...
	printf("==================================================================================\n");
	for(i = 0; i<s_http_Ctrl->i_conn_num; i++)
	{
...
static int Add_Syn_Http_Link()
{
	...
	dcs_log(0,0,"添加Http 同步通讯服务,queue_name=[%s]",queue_name);
	//先查找是否是已存在的链路
	nRet = Get_Syn_Http_Link(queue_name,&nFlag);
	if(nRet >=0)
	{
		dcs_log(0,0,"Http 同步通讯服务已经存在,queue_name=[%s]",queue_name);
		printf("Http 同步通讯服务已经存在,queue_name=[%s]\n",queue_name);
		return -1;
	}
	//不存在的话，从配置文件中 读出配置信息
	nRet=Reload_Syn_Http_Cfg(s_http_Ctrl,queue_name,1);
	if(nRet <0)
	{
		return -1;
	}

	nRet=Start_Process("synHttp",queue_name);
	if(nRet < 0)
	{
		dcs_log(0,0,"Http 同步通讯服务queue_name=[%s]添加失败",queue_name);
		printf("Http 同步通讯服务queue_name=[%s]添加失败\n",queue_name);
		return -1;
	}
	dcs_log(0,0,"Http 同步通讯服务queue_name=[%s]添加成功",queue_name);
	return 0;
}
...
static int Reload_Syn_Http_Cfg(struct http_Ast *pLink,char *pname,int mode)
{
	...
	if(mode == 1)
	{
		for(iSlotIndex=0;iSlotIndex<s_http_Ctrl->i_conn_num;iSlotIndex++)
		{
	...
	for( ;iRc == 0 && iSlotIndex < s_http_Ctrl->i_conn_num;iRc = conf_get_next_string(iFd, "conn",caBuffer))
	{
		...
		if(mode == 0)
		{
			iSlotIndex =Get_Syn_Http_Link(pname,&iRc);
		}
...
int Stop_Syn_Http_Link()
{
	...
	if(nRet == -1)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务不存在",queue_name);
		printf("Http 同步通讯服务不存在\n",queue_name);
		return 0;
	}
	else if(nFlag == DCS_STAT_STOPPED)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务已是停止状态",queue_name);
		printf("Http 同步通讯[%s] 服务已是停止状态\n",queue_name);
		return 0;
	}
	//链路存在,向链路进程发送终止信号
	...
	if(nRet == 0)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务关闭成功",queue_name);
	}
	else
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务关闭失败,err:%s",queue_name,strerror(errno));
		printf("Http 同步通讯[%s] 服务关闭失败,err:%s\n",queue_name,strerror(errno));
	}
...
*/
int Get_Syn_Http_Link(char *pname,int *pFlag)
{
	int i;
...
static int Open_Syn_Http_Cmd_Log()
{
    ...
    if(u_fabricatefile("log/synHttpCmd.log",logfile,sizeof(logfile)) < 0)
        return -1;

    return dcs_log_open(logfile, "synHttpCmd");
}


int  Start_Syn_Http_Link()
{
	char queue_name[20+1];
	char sBuf[200]={0};
	int nRet = -1;
	...
	dcs_log(0,0,"启动Http 同步通讯[%s] 服务",queue_name);
	nRet = Get_Syn_Http_Link(queue_name,&nFlag);
	if(nRet == -1)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务不存在",queue_name);
	...
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务已经在运行",queue_name);
	...
	if(nRet < 0)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务启动失败",queue_name);
		printf("Http 同步通讯[%s] 服务启动失败\n",queue_name);
	}
	dcs_log(0,0,"Http 同步通讯[%s] 服务启动成功",queue_name);
	return 0;
}

/*更新相关短链服务
*/
int Update_Syn_Http_Link()
{
	char queue_name[20+1];
	char sBuf[200]={0};
	int nRet = 0, nPid=0;
	...
	dcs_log(0,0,"更新Http 同步通讯[%s] 服务",queue_name);
	nRet = Get_Syn_Http_Link(queue_name,&nFlag);
	if(nRet == -1)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务不存在",queue_name);
	...
	//重新从配置文件中 读出相应配置信息
	nRet=Reload_Syn_Http_Cfg(s_http_Ctrl,queue_name,0);
	if(nRet <0)
	{
		dcs_log(0,0,"更新Http 同步通讯[%s] 服务失败",queue_name);
		printf("更新Http 同步通讯%s] 服务失败\n",queue_name);
		return -1;
	}	
	sleep(1);// 等待一秒，等待kill信号量先被处理
	nPid=Start_Process("synHttp",queue_name);
	if(nPid < 0)
	{
		dcs_log(0,0,"Http 同步通讯[%s] 服务启动失败",queue_name);
		printf("Http 同步通讯[%s] 服务启动失败\n",queue_name);
	...
	dcs_log(0,0,"更新Http 同步通讯[%s] 服务成功",queue_name);
	return 0;
}
