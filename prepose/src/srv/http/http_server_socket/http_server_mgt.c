...
int main(int argc, char *argv[]) 
{
	...
	dcs_log(0,0, "Http Server Mgt is starting up...");
	...
	dcs_log(0,0, "Http Server  config item count =%d",linkCnt);

	if(Create_Http_Server_Shm(linkCnt) < 0)
	{
	 	dcs_log(0,0, "Http Server Mgt Create shm err!");
		 exit(0);
	}

	dcs_log(0,0, "Http Server Mgt Create shm success!");
	//加载短连接配置文件
	if(Load_Http_Server_Cfg() < 0)
	{
		dcs_log(0,0,"Http Server Mgt load Server config err!");
		exit(0);
	}

	dcs_log(0,0,"**********************************************\n"
	            "       Http Server begin  startup all processes!\n"
	            "**********************************************\n");

	Start_Http_Server_All_Processes();

	dcs_log(0,0,"*************************************************\n"
			 "!******** Http servers startup completed.*********!!\n"
			"*************************************************\n");
...
static int Load_Http_Server_Cfg(void)
{
	...
	iRc = conf_get_next_string(iFd, "conn",caBuffer))
	{
		...
		dcs_log(0, 0, "name = [%s], fold_name = [%s], msgtype = [%s], port = [%s]\n",
			name,fold_name,msgtype,port);
...
static int Open_Http_Server_Mgt_Log(char *IDENT)
{
	...
	if(u_fabricatefile("log/httpSvr.log",logfile,sizeof(logfile)) < 0)
		return -1;
...
int Create_Http_Server_Shm(int likCnt)
{
	...
	ptr = shm_create(HTTP_SERVER_SHM_NAME, size,&shmid);
...
static int Start_Http_Server_All_Processes()
{
	...
	for(i=0;i< p_http_s_Ctrl->i_http_num;i++)
	{
		...
		if(sc_link.port > 0 )
		{
			...
			Start_Process("httpServer",sc_link.name);
