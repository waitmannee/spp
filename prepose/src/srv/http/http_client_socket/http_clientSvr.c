...
static struct http_c_Ast *p_http_c_Ctrl = NULL;
//======= Funtion Forward delection =============//
static int OpenHttpClientLog(char *IDENT);
static int load_http_client_cfg();
static int create_http_client_shm(int likCnt);
static int load_http_client_cfg_cnt();
static int StartHttpClientAllProcesses();
//==============================================//
...
int main(int argc, char *argv[]) 
{
	...
	if(0 > OpenHttpClientLog(argv[0])) exit(1);

	dcs_log(0,0, "Http Client Svr is starting up...");

	u_daemonize(NULL);

	int linkCnt = load_http_client_cfg_cnt();
	//创建短链接客户端的共享内存

	dcs_log(0,0, "Http Client  config item count =%d",linkCnt);

	if(create_http_client_shm(linkCnt) < 0)
	{
	 	dcs_log(0,0, "Http Client Serv Create shm err!");
		 exit(0);
	}

	dcs_log(0,0, "Http Client Serv Create shm success!");
	//加载短连接配置文件
	if(load_http_client_cfg() < 0)
	{
		dcs_log(0,0,"Http Client Serv load Client config err!");
		exit(0);
	}

	dcs_log(0,0,"Http Client Serv load  config success!");

	dcs_log(0,0,"**********************************************\n"
	            "       Http Client Serv  Start all processes!\n"
	            "**********************************************\n");

	StartHttpClientAllProcesses();

	dcs_log(0,0,"*************************************************\n"
			 "!******** Http clients startup completed.*********!!\n"
			"*************************************************\n");
 ...
 * 加载http短链接客户端的配置文件
 */
static int load_http_client_cfg(void)
{
	char   caBuffer[512];
	...
	for( ;
	iRc == 0 && iSlotIndex < p_http_c_Ctrl->i_http_num;
...
static int OpenHttpClientLog(char *IDENT)
{
	...
	if(u_fabricatefile("log/httpClient.log",logfile,sizeof(logfile)) < 0)
		return -1;
 ...
 * 创建http客户端的共享内存
 */
int create_http_client_shm(int likCnt)
{
	char *ptr = NULL;
	int shmid =-1;
	int size = likCnt * sizeof(struct http_c_Ast);

	dcs_log(0, 0, "shm create http_client SHM size = %d ", size);
	ptr = shm_create(HTTP_CLIENT_SHM_NAME, size,&shmid);
	dcs_log(0, 0, "shm create http_client shm_id = %d ", shmid);
	if(ptr == NULL)
	{
	...
	p_http_c_Ctrl   = (struct http_c_Ast *)ptr;
...
{
	int i;
	for(i=0;i< p_http_c_Ctrl->i_http_num;i++)
	{
		struct http_c sc_link = p_http_c_Ctrl->httpc[i];
		if(strlen(p_http_c_Ctrl->httpc[i].serverAddr))
		{
			dcs_log(0,0,"name =[%s],fold_name = [%s],iApplFid=[%d],iFid=[%d],serverAddr=[%s]",sc_link.name,sc_link.fold_name,sc_link.iApplFid,sc_link.iFid,sc_link.serverAddr);
			Start_Process("httpClient",sc_link.name);
